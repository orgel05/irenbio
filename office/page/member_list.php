...
// 첫페이지
$page_list = "<a href=\"/bio/office/page/member_list.php?page=1&s_que=$s_que&sh_type=$sh_type&mode=$mode&utype=$utype&so_name=$so_name&so_arr=$so_arr\" title=\"첫 페이지\" class=\"plast\" rel=\"external\"><img src=\"/bio/officedemo/img/bl_left2.gif\" alt=\"첫페이지\" /></a>";

// 현재 페이지영역 이전의 페이지영역이 있는 경우 이전페이지목록으로 가는 링크
if (($pagebox_cur * $pagebox) >= $pagebox) {
	$page_list .= "<a href='/bio/office/page/member_list.php?page=".$a."&s_que=$s_que&sh_type=$sh_type&mode=$mode&utype=$utype&so_name=$so_name&so_arr=$so_arr' title=\"이전 페이지그룹\" class=\"pnext\" rel=\"external\"><img src=\"/bio/officedemo/img/bl_left.gif\" alt=\"이전 $pagebox 페이지\" /></a>";
}

// 페이지영역안에 들어오는 경우에 페이지번호 뿌려주면서 해당페이지가 현재페이지인지 체크
for ($i = $b; ($i <= $pagemax) && ($i <= $c); $i ++) {
	if ($page != $i) {
		$page_list .= "<a href='/bio/office/page/member_list.php?page=$i&s_que=$s_que&sh_type=$sh_type&mode=$mode&utype=$utype&so_name=$so_name&so_arr=$so_arr' title=\"$i 페이지\" class=\"num\" rel=\"external\">$i</a>";
    } else {
...
if ($c < $pagemax) {
      $page_list .= "<a href='/bio/office/page/member_list.php?page=".$d."&s_que=$s_que&sh_type=$sh_type&mode=$mode&utype=$utype&so_name=$so_name&so_arr=$so_arr' title=\"다음 페이지그룹\" class=\"pnext\" rel=\"external\"><img src=\"/bio/officedemo/img/bl_right.gif\" alt=\"다음 $pagebox 페이지\" /></a>";
}

$page_list .= "<a href=\"/bio/office/page/member_list.php?page=$pagemax&s_que=$s_que&sh_type=$sh_type&mode=$mode&utype=$utype&so_name=$so_name&so_arr=$so_arr\" title=\"마지막 페이지\" class=\"plast\" rel=\"external\"><img src=\"/bio/officedemo/img/br_right2.gif\" alt=\"마지막페이지\" /></a>";
...
?>
	...
	<script type="text/javascript">
	    ...
	    function go_view(no){
	    	location.href="/bio/office/page/member_view.php?idx="+no;

	    }
	    function go_add() {
	    	location.href="/bio/office/page/member_add.php";
	    	// body...
	    }
	</script>
	<style type="text/css">
...
<body>
	...
	<div class="masterBody" id="masterBody">
		...
		$sql = "select * from a_mem where view='Y' ";
		$result = mysql_query($sql, $connect) ;
...
?>
        <div class="contentArea">
        	<table cellpadding="0" cellspacing="0" class="AXFormTable">
        		...
        		<tr>
        			<th><div class="tdRel">번호</div></th>
        			<th><div class="tdRel">ID</div></th>			
        			<th><div class="tdRel">이름</div></th>
        			<th><div class="tdRel">레벨</div></th>
        			<th><div class="tdRel">부서</div></th>
        			<th><div class="tdRel">직책</div></th>
        			<th><div class="tdRel">전화번호</div></th>
		...
		?>	
			...
			<div class="bnt_go">
				<button onclick="go_add()"   class="AXButton Blue">추가하기</button>
			</div>
		</div>
	</div>
</body>
