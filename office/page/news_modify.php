...
$idx = $_GET['idx'];
$sql = "select * from iren_news where '$idx' =no";
...
if($_POST['mode']=='up'){
	...
	$sql = "UPDATE iren_news SET title='$title',date='$p_date',contents='$contents' WHERE no='$idx'";
...
<form action="news_modify.php?idx=<?=$row['no']?>s" method="post" name="insert" " enctype="multipart/form-data">
	<div style="width:776px;margin-bottom:15px; ">
	<label>제목 : </label>	<input type="text" name="title" class="AXInput" style="width:730px;border:1px solid gray;" value="<?=$row['title']?>">
	</div>
