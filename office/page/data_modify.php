...
$idx = $_GET['idx'];
$sql = "select * from iren_data where '$idx' =no";
...
if($_POST['mode']=='up'){
	...
	$sql = "UPDATE iren_data SET title='$title',date='$p_date',contents='$contents' WHERE no='$idx'";

	   $result=mysql_query($sql,$connect);
	   echo '<script>location.href="data_list.php"</script>';
...
<form action="data_modify.php?idx=<?=$row['no']?>s" method="post" name="insert" " enctype="multipart/form-data">
	<div style="width:776px;margin-bottom:15px; ">
	<label>제목 : </label>	<input type="text" name="title" style="width:730px;border:1px solid gray;" value="<?=$row['title']?>">
	</div>
	<textarea name="ir1" id="ir1" rows="10" cols="100" style="width:766px; height:412px; display:none;"><?=$row['title']?></textarea>
