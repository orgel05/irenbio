<?php
	...
	//$row = mysql_fetch_array($result);to_char(date,'YYYY-MM-DD')
...
// 첫페이지
$page_list = "<a href=\"/bio/support02.php?page=1&s_que=$s_que&sh_type=$sh_type&mode=$mode&utype=$utype&so_name=$so_name&so_arr=$so_arr\" title=\"첫 페이지\" class=\"plast\" rel=\"external\"><img src=\"/bio/officedemo/img/bl_left2.gif\" alt=\"첫페이지\" /></a>";

// 현재 페이지영역 이전의 페이지영역이 있는 경우 이전페이지목록으로 가는 링크
if (($pagebox_cur * $pagebox) >= $pagebox) {
	$page_list .= "<a href='/bio/support02.php?page=".$a."&s_que=$s_que&sh_type=$sh_type&mode=$mode&utype=$utype&so_name=$so_name&so_arr=$so_arr' title=\"이전 페이지그룹\" class=\"pnext\" rel=\"external\"><img src=\"/bio/officedemo/img/bl_left.gif\" alt=\"이전 $pagebox 페이지\" /></a>";
}

// 페이지영역안에 들어오는 경우에 페이지번호 뿌려주면서 해당페이지가 현재페이지인지 체크
for ($i = $b; ($i <= $pagemax) && ($i <= $c); $i ++) {
	if ($page != $i) {
		$page_list .= "<a href='/bio/support02.php?page=$i&s_que=$s_que&sh_type=$sh_type&mode=$mode&utype=$utype&so_name=$so_name&so_arr=$so_arr' title=\"$i 페이지\" class=\"num\" rel=\"external\">$i</a>";
    } else {
...
if ($c < $pagemax) {
      $page_list .= "<a href='/bio/support02.php?page=".$d."&s_que=$s_que&sh_type=$sh_type&mode=$mode&utype=$utype&so_name=$so_name&so_arr=$so_arr' title=\"다음 페이지그룹\" class=\"pnext\" rel=\"external\"><img src=\"/bio/officedemo/img/bl_right.gif\" alt=\"다음 $pagebox 페이지\" /></a>";
}
...
<div id= "contentsWrap">
    ...
    while ($row = mysql_fetch_array($result)) {
    	$memo = strip_tags($row[memo]);
			...
			?>
		<tr onmouseover="this.style.background='#E8EEC6'" onmouseout="this.style.background='#ffffff'" onclick="go_data('<?=$row['no']?>');" style="cursor: pointer;">
			<td class="data_list"><?=$row['no']?></td>
			<td class="data_list" style="text-align:left; padding-left:20px;"><?=$row['title']?></td>
			<td class="data_list"><?=$row['p_id']?></td>
...
<script type="text/javascript">
	function go_data(no){
		location.href="/bio/support02_1.php?idx="+no;
